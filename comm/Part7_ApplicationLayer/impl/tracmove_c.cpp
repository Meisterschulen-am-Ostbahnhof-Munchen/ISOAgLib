...
namespace __IsoAgLib { // Begin Namespace __IsoAglib
  ...
  { // if > 1 singleton instance is used, no static reference can be used
    MACRO_MULTITON_GET_INSTANCE_BODY(TracMove_c, PRT_INSTANCE_CNT, instance);
  }

  bool TracMove_c::config_base ( IdentItem_c* apc_ident, IsoAgLib::IdentMode_t at_identMode, uint16_t aui16_suppressMask)
  {
    ...
    if (t_oldMode == at_identMode)
      ...
      if (canSendSelectedSpeed())
        s_register(MACHINE_SELECTED_SPEED_PGN);
      ...
      if (canSendSelectedSpeed())
        s_unregister(MACHINE_SELECTED_SPEED_PGN);
      if (canSendEngineSpeed())
  ...
  void TracMove_c::checkCreateReceiveFilter()
  {
    ...
    { // check if needed receive filters for ISO are active
      ...
      c_can.insertFilter( *this, IsoAgLib::iMaskFilter_c( 0x3FFFF00UL, (MACHINE_SELECTED_SPEED_PGN<<8) ), 8 );
  ...
  void TracMove_c::processMsg( const CanPkg_c& arc_data )
  {
    ...
    switch (pkg.isoPgn() /*& 0x3FFFF*/) // don't need to &0x3FFFF as this is the whole PGN...
    {
      ...
      case WHEEL_BASED_SPEED_DIST_PGN:
        ...
        break;
      case MACHINE_SELECTED_SPEED_PGN:
        {
          // only take values, if i am not the regular sender
          // and if actual sender isn't in conflict to previous sender
  ...
  TracMove_c::SendMessage_e TracMove_c::sendSelectedSpeed()
  {
    ...
    CanPkgExt_c pkg;
    prepareSending( pkg );
    pkg.setIsoPgn(MACHINE_SELECTED_SPEED_PGN);
    uint8_t ui8_temp = 0;
