...
/* ******************************************************** */
/**
 * \name DEFAULT_POWERDOWN_STRATEGY Default strategy which is used by
...
/** select Power Down trigger source:
		...
		- IsoAgLib::PowerdownOnCanEnLoss for automatic stop of ECU on detection of
				CAN_EN loss
	*/
#ifndef DEFAULT_POWERDOWN_STRATEGY
...
/** define longest token which is used to translate printed
    numbers to number variables
*/
 ...
 */
/*@{*/
/* uncomment the config setting to use for EEEdit */
/**
  * @def USE_CAN_EEPROM_EDITOR_YN
  * use CAN connected EEPROM editor
  */
  #ifndef USE_CAN_EEPROM_EDITOR_YN
    #define USE_CAN_EEPROM_EDITOR_YN NO
  #endif
  #if USE_CAN_EEPROM_EDITOR_YN == YES && !defined(USE_CAN_EEPROM_EDITOR)
    #define USE_CAN_EEPROM_EDITOR
  #endif

/**
  * @def USE_RS232_EEPROM_EDITOR_YN
  * use RS232 connected EEPROM editor
  */
  #ifndef USE_RS232_EEPROM_EDITOR_YN
    #define USE_RS232_EEPROM_EDITOR_YN NO
  #endif
  #if USE_RS232_EEPROM_EDITOR_YN == YES && !defined(USE_RS232_EEPROM_EDITOR)
 ...
 * Define time between subsequent timeEvent calls
...
/** time to delete inactive member items in monitor list [msec]
    - if MEMBER_ITEM_MAX_AGE is not defined, no items are deleted
*/
//#define MEMBER_ITEM_MAX_AGE 3000
/** time to delete inactive service items in monitor list [msec]
    - if SERVICE_ITEM_MAX_AGE is not defined, no items are deleted
*/
#define SERVICE_ITEM_MAX_AGE 3000
/** time to delete inactive ISO items in monitor list [msec]
    - if ISO_ITEM_MAX_AGE is not defined, no items are deleted
*/
...
#endif
/// define CAN BUS number used for IsoAgLib (setting of 0xFF forces explicit call
/// of init, to open the CAN BUS )
#define DEFAULT_BUS_NUMBER 0xFF
/// define default MsgObj_c number to use for CAN send
#define DEFAULT_SEND_OBJ HAL_CAN_MSGOBJ_OFFSET
...
#define DEFAULT_MIN_OBJ_NR HAL_CAN_MSGOBJ_OFFSET
/// define default max number for CAN Msg-Obj used for IsoAgLib
#define DEFAULT_MAX_OBJ_NR ( HAL_CAN_MSGOBJ_OFFSET + 13 )
/*@}*/
...
#define RECEIVE_PUFFER_SIZE 30
/** DIN target msgs are sent with high rate -> need greater buffer */
#define RECEIVE_DIN_TARGET_PUFFER_SIZE 35
...
#define RECEIVE_DIN_PARTNER_PUFFER_SIZE 20
  ...
  */
  #define FILTER_BOX_PER_MSG_OBJ 15
#else
  /** configure the amount of max filterBox instances to one msgObj instance
      static array for better quicker access to data.
  ...
  */
  #define FILTER_BOX_PER_MSG_OBJ 5
#endif
/*@}*/
...
/** max time intervall of CAN err before switching
   back from 250 to 125 kbit/s in [msec.]
*/
...
#define DEFAULT_SND_PUF_SIZE 100
