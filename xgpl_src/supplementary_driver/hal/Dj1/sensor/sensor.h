...
//    return ( 0 );                      /* TBD - Not yet supported (c2c) 1ms scan, 5ms Avg */
  ...
  */
  ...
  {
    int16_t Value = __HAL::DjBios_DigGetState ( rb_channelNumber );
    ...
    else /* if ( Value == BIOS_UART_BAD_CHAN ) */
    {
  ...
  */
  ...
  {
    int16_t Value = __HAL::DjBios_DigGetStateDebounce ( rb_channelNumber );
    if ( Value == __HAL::BIOS_DIG_HIGH )
    {
      return ( ON );
    } /* end if() */
