...
/* ********************************************************** */
/** \file supplementary_driver/hal/esx/actor/actor_target_extensions.cpp
...
static const uint16_t cui16_openLow_4000mA_13_17  = ( 170000 / 900 ); // 1700mV * 100 / 9mV * 100
  ...
  int16_t getDigoutDiagnose(uint8_t rui8_channel, uint16_t rui16_minCurrent, uint16_t rui16_maxCurrent)
  {
    ...
    #else
    const int16_t i16_adc = (rui8_channel < 12)
      ?__HAL::get_adc(__HAL::getPwmoutAdcCheckNr(rui8_channel))
      :__HAL::BA_get_adc( POSITION_1, (rui8_channel-12) );;
    #endif

		const bool b_hiSet = ( rui16_maxCurrent > 0 )?true:false;
		uint16_t ui16_useLowCompare = (rui8_channel<8)?cui16_openLow_4000mA_6_8:cui16_openLow_2500mA_9_12;
    #ifdef _INIT_BABYBOARD_
    if (rui8_channel>11) ui16_useLowCompare = cui16_openLow_4000mA_13_17;
    #endif
	...
	*/
    ...
    {
      const int16_t ci16_result = __HAL::BA_get_adc( POSITION_1, (rui8_channel-12) );
      if ( ci16_result != HAL_NO_ERR ) return ci16_result;
