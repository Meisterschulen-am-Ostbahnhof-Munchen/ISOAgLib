...
#ifndef _HAL_PC_SENSOR_H_
#define _HAL_PC_SENSOR_H_
...
namespace HAL
{
  ...
  */
  ...
  /**
    ...
    @param rb_channel channel of counter
    @return C_NO_ERR ; HAL_RANGE_ERR if counter for rb_channel isnt configured properly
  */
  ...
  */
  inline int16_t  getAdcVoltage(uint8_t rb_channel)
    {int16_t i16_temp = __HAL::getAdc(__HAL::getAnaloginCheckNr(rb_channel));
  ...
  */
  inline int16_t  getAdcCurrent(uint8_t rb_channel)
    {int16_t i16_temp = __HAL::getAdc(__HAL::getDiginAdcCheckNr(rb_channel));
     if ( i16_temp == HAL_RANGE_ERR ) return HAL_RANGE_ERR;
     // use 1:1 value mappgin for PC test
  ...
  */
  inline bool  getAdcVirtMass( void )
    {return (__HAL::getAdc(GET_M_VIRT) < 440 )?true:false;};
  /**
    get the temperature
