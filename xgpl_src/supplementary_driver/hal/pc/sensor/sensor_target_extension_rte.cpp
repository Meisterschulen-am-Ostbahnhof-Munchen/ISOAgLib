/***************************************************************************
                          sensor_target_extensions.cpp - source for PC specific
...
#include "sensor_target_extension_rte.h"
#include <stdlib.h>
#include <ctype.h>
...
struct sensor_channel_dimensions_s {
  enum index_e {
    begin = channel_dimensions_s::input_begin,
    end = channel_dimensions_s::input_end
  };
  enum count_e {
    num = end - begin
  };
...
static din_c channelDigitalInput[sensor_channel_dimensions_s::num];

static bool sensorAnalogInputOpen[] = { false,false,false,false,false,false,false,false };
static bool sensorDigitalInputOpen[sensor_channel_dimensions_s::num];

static int32_t lastSensorAnalogInputTime[ANALOG_INPUT_MAX+1];
static int32_t lastSensorAnalogInputVal[ANALOG_INPUT_MAX+1];
static bool lastSensorDigitalInputVal[sensor_channel_dimensions_s::end];
...
template <>
void report_bit_state<sensor_channel_dimensions_s>(
  uint8_t logical_channel_number,
  bool bit_state)
{
  lastSensorDigitalInputVal[logical_channel_number] = bit_state;
}
...
int16_t  init_digin(uint8_t bInput,uint8_t bMode,uint8_t bAktivhighlow,void (*pfFunctionName)(...)){
  ...
  bool got_channel = get_rte_channel<sensor_channel_dimensions_s>(rte_channel, bInput);
  if (got_channel) {
    din_c &r_c = channelDigitalInput[bInput];
    r_c.set_channel(rte_channel);
    r_c.set_state_handler(dinout_state<sensor_channel_dimensions_s>);
...
static int16_t getDiginOnoffCommon(uint8_t bInputNumber)
{
  if (!sensorDigitalInputOpen[bInputNumber])
    return HAL_CONFIG_ERR;
  rte_poll();
  return int16_t(lastSensorDigitalInputVal[bInputNumber]);
}

int16_t getDiginOnoff(uint8_t bInputNumber)
{
  // printf("getDiginOnoff(%i)\n", bInputNumber);
  return getDiginOnoffCommon(bInputNumber);
}
...
//  int32_t ui16_prescale = ((rui16_timebase * getCpuFreq() * 1000) / 65534);
  ...
  { /* according 4-group of uint32_t isnt created -> allocate */
  ...
  { /* standard BIOS frequency and period methods doesnt fir for
     * the wanted timebase -> use extension functions -> allocate needed vars
     */
    if (_pt_diginTriggerTime[(rb_channel / 4)] == NULL)
    {  /* according 4-group of t_triggerNode isnt created -> allocate */
...
/**
  ...
  @param rb_channel channel of counter
  @return HAL_NO_ERR ; HAL_RANGE_ERR if counter for rb_channel isnt configured properly
*/
