...
public:
  ...
  */
  void init(const IsoAgLib::ElementDDI_s* ps_elementDDI, uint16_t rui16_element,
            const DevKey_c& rc_devKey = DevKey_c::DevKeyInitialProcessData, uint8_t rui8_pri = 2,
            const DevKey_c& rc_ownerDevKey = DevKey_c::DevKeyUnspecified,
            const DevKey_c* rpc_commanderDevKey = NULL,
            IsoAgLib::ProcessDataChangeHandler_c *rpc_processDataChangeHandler = NULL,
            int ri_singletonVecKey = 0);
  ...
  */
  const ProcDataRemoteSimpleMeasure_c& operator=(const ProcDataRemoteSimpleMeasure_c& rrefc_src);

  /**
    copy constructor for IsoAgLibProcDataRemote
    @param rrefc_src source instance
  */
   ProcDataRemoteSimpleMeasure_c(const ProcDataRemoteSimpleMeasure_c& rrefc_src);
