...
int32_t ProcDataRemote_c::masterMeasurementVal(bool rb_sendRequest)
{
  ...
  { // setting of devKey in MeasureProg is normally done via ProcDataRemote_c::timeEvent( void )
    // if request follows immediately init, timeEvent is not called yet => do it here
    ...
    if (commanderDevKey() != DevKey_c::DevKeyUnspecified)
      c_measure.setDevKey(commanderDevKey());
  }
#endif
