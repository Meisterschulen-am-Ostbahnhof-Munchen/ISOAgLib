        ...
        en_command == GeneralCommand_c::measurementMaximumThresholdValueStart)
    {
      ...
      if (i32_dataLong != 0)
      {
        if (en_typePkg != Proc_c::NullType)
          start(static_cast<Proc_c::progType_t>(c_pkg.pri()), en_typePkg, en_doSendPkg);
      }
      else
...
void MeasureProgBase_c::processIncrementMsg(Proc_c::doSend_t ren_doSend){
  ...
  int32_t i32_valPkg = c_pkg.dataRawCmdLong();
  ...
  if (c_pkg.c_generalCommand.getCommand() == GeneralCommand_c::measurementTimeValue ||
    ...
    // time proportional
    addSubprog(Proc_c::TimeProp, CNAMESPACE::labs(i32_valPkg), ren_doSend);
  ...
  if (c_pkg.c_generalCommand.getCommand() == GeneralCommand_c::measurementDistanceValue ||
    ...
    addSubprog(Proc_c::DistProp, i32_valPkg, ren_doSend);
