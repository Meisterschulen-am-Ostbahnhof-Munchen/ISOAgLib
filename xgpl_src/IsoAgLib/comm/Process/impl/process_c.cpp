...
#ifdef USE_ISO_11783
                        , data().memberSend().devKey()
                        , data().memberEmpf().devKey()
#endif
                          )
      )
...
)
{
  ...
  { // search for all local items which match the searched identity
    ...
    if (data().identType() == Ident_c::StandardIdent) {
      ...
      if ((*pc_iter)->matchISO(rui8_devClass, 0xFF, rui16_DDI, rui16_element, rref_devKey, DevKey_c::DevKeyUnspecified))
        ui8_cnt++;
    }
#else
  #ifdef USE_ISO_11783
...
)
{
  ...
  { // search for all local items which match the searched identity
    ...
    if (data().identType() == Ident_c::StandardIdent) {
      ...
      // ISO
      if ((*pc_iter)->matchISO(rui8_devClass, rui8_devClassSender, rui16_DDI, rui16_element, rref_devKey, rref_devKeySender))
        ui8_cnt++;
    }
#else
  #ifdef USE_ISO_11783
  ...
  if (!c_arrClientC1.empty())
  {
    if ( pc_searchCacheC1 != c_arrClientC1.end() )
    {
      ...
      if (data().identType() == Ident_c::StandardIdent) {
        ...
        // ISO
        // don't check sender devClass => 0xFF
        if ((*pc_searchCacheC1)->matchISO(rui8_devClassReceiver, 0xFF, rui16_DDI, rui16_element, rref_devKeyReceiver, DevKey_c::DevKeyUnspecified)) return true;
      }
#else
  #ifdef USE_ISO_11783
      // don't check sender devClass => 0xFF
    ...
    { // check for lazy match with INSTANCE == 0xFF (==joker)
      ...
      if (data().identType() == Ident_c::StandardIdent) {
        ...
        // ISO
        // don't check sender devClass => 0xFF
        if ((*pc_iter)->matchISO(rui8_devClassReceiver, 0xFF, rui16_DDI, rui16_element, rref_devKeyReceiver, DevKey_c::DevKeyUnspecified))
          b_matched = true;
      }
#else
  #ifdef USE_ISO_11783
  ...
  if (!c_arrClientC2.empty())
  {
    if ( pc_searchCacheC2 != c_arrClientC2.end() )
    {
      ...
      if (data().identType() == Ident_c::StandardIdent) {
        ...
        // ISO
        if ((*pc_searchCacheC2)->matchISO(rui8_devClassReceiver, rui8_devClassSender, rui16_DDI, rui16_element, rref_devKeyReceiver, rref_devKeySender)) return true;
      }
#else
  #ifdef USE_ISO_11783
      if ((*pc_searchCacheC2)->matchISO(rui8_devClassReceiver, rui8_devClassSender, rui16_DDI, rui16_element, rref_devKeyReceiver, rref_devKeySender)) return true;
    ...
    { // check for lazy match with INSTANCE == 0xFF (==joker)
      ...
      if (data().identType() == Ident_c::StandardIdent) {
        ...
        // ISO
        if ((*pc_iter)->matchISO(rui8_devClassReceiver, rui8_devClassSender, rui16_DDI, rui16_element, rref_devKeyReceiver, rref_devKeySender))
          b_matched = true;
      }
#else
  #ifdef USE_ISO_11783
...
bool Process_c::processTcStatusMsg(uint8_t ui8_tcStatus, const DevKey_c& refc_devKey, bool rb_skipLastTcStatus)
{
  ...
  { // call handler function if handler class is registered
    pc_processDataChangeHandler->processTcStatusMessage((1 == ui8_tcStatus) /* 1: task running */, refc_devKey);
  }
