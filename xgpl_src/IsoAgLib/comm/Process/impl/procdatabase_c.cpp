...
/**
  ...
  */
void ProcDataBase_c::init(GetyPos_c rc_gtp, uint16_t rui16_DDI,
      uint16_t rui16_element, uint8_t rui8_pri, GetyPos_c rc_ownerGtp, GetyPos_c *rpc_gtp,
      IsoAgLib::ProcessDataChangeHandler_c *rpc_processDataChangeHandler,
      int ri_singletonVecKey)
{
  ProcIdent_c::init(rc_gtp, rui16_DDI, rui16_element, rui8_pri, rc_ownerGtp, rpc_gtp);
...
void ProcDataBase_c::processMsg(){
 ...
 // printf("pd() returns %d\n", getProcessInstance4Comm().data().pd());
 // printf("mod() returns %d\n", getProcessInstance4Comm().data().mod());
printf("processDataBase msg pd: %X  mod: %X \n", getProcessInstance4Comm().data().pd(), getProcessInstance4Comm().data().mod() );  // 3 test message Wayne
...
//printf("processDataBase processed **wrong** msg \n" );  // 3 test message Wayne
...
printf("processDataBase processed msg \n" );  // 3 test message Wayne
...
#else // else if ISO_TASK_CONTROLLER not defined
/**
  initialise this ProcDataBase_c instance to a well defined initial state
