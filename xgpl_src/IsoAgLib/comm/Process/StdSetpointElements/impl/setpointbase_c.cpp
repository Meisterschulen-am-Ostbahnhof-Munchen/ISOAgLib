...
void SetpointBase_c::processMsg(){
  ...
  if (getProcessInstance4Comm().data().pd() == 0x2)
  {
    processRequest();
  }
  else
                                       ...
                                       bool b_override, int32_t ri32_overrideVal) const
{
  ...
  { // exact setpoint exist -> send it
    if (b_override)i32_value = ri32_overrideVal;
    else i32_value = rrefc_src.exact();
    if (b_isCmd){
    ...
    if (rrefc_src.existMin())
    { // min setpoint exist -> send it
    ...
    if (rrefc_src.existMax())
    { // max setpoint exist -> send it
