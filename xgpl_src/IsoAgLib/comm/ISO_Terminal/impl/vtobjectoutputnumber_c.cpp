...
#define MACRO_vtObjectTypeS iVtObjectOutputNumber_s
    ...
    if (sourceOffset == 0) { // dump out constant sized stuff
      ...
      if (flags & FLAG_ORIGIN_SKM) {
        destMemory [3] = (((uint32_t) vtObjectOutputNumber_a->width * factor) >> 20) & 0xFF;
        destMemory [4] = (((uint32_t) vtObjectOutputNumber_a->width * factor) >> 20) >> 8;
        destMemory [5] = (((uint32_t) vtObjectOutputNumber_a->height * factor) >> 20) & 0xFF;
        destMemory [6] = (((uint32_t) vtObjectOutputNumber_a->height * factor) >> 20) >> 8;
      } else {
      ...
      destMemory [7] = vtObjectOutputNumber_a->backgroundColour;
...
void
vtObjectOutputNumber_c::setValue(uint32_t newValue, bool b_updateObject)
{ // ~X2C
  if (get_vtObjectOutputNumber_a()->variableReference == NULL) {
    ...
    // Send Value update
    __IsoAgLib::getIsoTerminalInstance().sendCommandChangeNumericValue (this, newValue & 0xFF, (newValue >> 8) & 0xFF, (newValue >> 16) & 0xFF, newValue >> 24);
  }
} // -X2C
...
void
vtObjectOutputNumber_c::setSize(uint16_t newWidth, uint16_t newHeight, bool b_updateObject)
{
  if (b_updateObject) {
