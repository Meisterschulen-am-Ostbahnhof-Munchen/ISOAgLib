...
#define MACRO_vtObjectTypeS iVtObjectLine_s
    ...
    if (sourceOffset == 0) { // dump out constant sized stuff
      ...
      if (flags & FLAG_ORIGIN_SKM) {
        ...
        destMemory [8] = (((uint32_t) vtObjectLine_a->height * factor) >> 20) >> 8;
      } else {
...
void
vtObjectLine_c::setEndPoint(uint16_t newWidth, uint16_t newHeight, uint8_t newLineDirection, bool b_updateObject)
{
  if (b_updateObject) {
    saveValue16 (MACRO_getStructOffset(get_vtObjectLine_a(), width),  sizeof(iVtObjectLine_s), newWidth);
    saveValue16 (MACRO_getStructOffset(get_vtObjectLine_a(), height), sizeof(iVtObjectLine_s), newHeight);
  ...
  __IsoAgLib::getIsoTerminalInstance().sendCommandChangeEndPoint (this, newWidth, newHeight, newLineDirection);
}
