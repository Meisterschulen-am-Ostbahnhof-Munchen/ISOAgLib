...
#define MACRO_vtObjectTypeS iVtObjectOutputString_s
    ...
    if (sourceOffset == 0) { // dump out constant sized stuff
      ...
      if (flags & FLAG_ORIGIN_SKM) {
        ...
        destMemory [6] = (((uint32_t) vtObjectOutputString_a->height * factor) >> 20) >> 8;
      } else {
      ...
      destMemory [7] = vtObjectOutputString_a->backgroundColour;
    ...
    while ((sourceOffset >= 16) && (sourceOffset < (16+vtObjectOutputString_a->length)) && ((curBytes+1) <= maxBytes)) {
    ...
    if ((sourceOffset == (16 + vtObjectOutputString_a->length)) && ((curBytes+1) <= maxBytes)) {
      destMemory [curBytes] = vtObjectOutputString_a->numberOfMacrosToFollow;
      curBytes++;
      sourceOffset++;
    }

    MACRO_streamEventMacro(17+vtObjectOutputString_a->length);
    return curBytes;
} // -X2C
...
{ // ~X2C
  ...
  pc_stringToStream = newValue; // use MultiSendStreamer with pc_stringToStream set!
  ui16_strLenToSend = (CNAMESPACE::strlen (newValue) <= get_vtObjectOutputString_a()->length) ? CNAMESPACE::strlen (newValue) : get_vtObjectOutputString_a()->length;
