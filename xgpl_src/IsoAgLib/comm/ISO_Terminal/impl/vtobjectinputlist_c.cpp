...
void
vtObjectInputList_c::setValue(uint8_t newValue, bool b_updateObject)
{ // ~X2C
  if (get_vtObjectInputList_a()->variableReference == NULL) {
    ...
    __IsoAgLib::getIsoTerminalInstance().sendCommandChangeNumericValue (this, newValue, 0x00, 0x00, 0x00);
  }
} // -X2C
...
//! @param rpc_object:
void
vtObjectInputList_c::setItem(uint8_t rui8_index, IsoAgLib::iVtObject_c* rpc_object)
{ // ~X2C
  uint8_t hi, lo;
  if (rpc_object == NULL) {
    lo = 0xFF;
  ...
  __IsoAgLib::getIsoTerminalInstance().sendCommand (177 /* Command: Command --- Parameter: Change List Item */,
                                                   this->getID() & 0xFF,
                                                   this->getID() >> 8,
                                                   rui8_index,
                                                   lo,
...
void
vtObjectInputList_c::setSize(uint16_t newWidth, uint16_t newHeight, bool b_updateObject)
{
  if (b_updateObject) {
