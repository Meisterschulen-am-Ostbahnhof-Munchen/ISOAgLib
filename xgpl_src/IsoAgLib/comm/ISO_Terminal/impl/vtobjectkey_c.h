...
public:
  ...
  //! @param b_initPointer:
  void init(const iVtObjectKey_s* vtObjectKeySROM) { vtObject_c::init ((iVtObject_s*) vtObjectKeySROM); };
  ...
  void setBackgroundColour(uint8_t newValue, bool b_updateObject=false) {
    saveValue8SetAttribute ((b_updateObject) ? MACRO_getStructOffset(get_vtObjectKey_a(), backgroundColour) : 0, sizeof(iVtObjectKey_s), 1, newValue, __IsoAgLib::getIsoTerminalInstance().getUserClippedColor (newValue, this, IsoAgLib::BackgroundColour));
  }
  void setKeyCode(uint8_t newValue, bool b_updateObject=false) {
    saveValue8SetAttribute ((b_updateObject) ? MACRO_getStructOffset(get_vtObjectKey_a(), keyCode) : 0, sizeof(iVtObjectKey_s), 2, newValue, newValue);
  }
