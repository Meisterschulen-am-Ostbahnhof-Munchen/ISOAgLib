...
IdentItem_c::IdentItem_c(DevKey_c* rpc_devKey,
  ...
  : BaseItem_c(System_c::getTime(), IState_c::Active, ri_singletonVecKey )
    ...
    #endif
    , b_wantedSa( 254 ), ui8_lastUsedSa( 0xFF ), ui16_saEepromAdr( 0xFFFF )
  #endif
...
void IdentItem_c::restartAddressClaim()
{
  ...
  { // item has claimed address -> send unregister cmd
    ...
    pc_memberItem = NULL;
// @todo3: remove inserted filter (from above)
  ...
  // following code is used independent of setting of CHANGE_DEV_CLASS_INST_ON_CONFLICT
  {
    ...
    if ( (pc_memberItem != NULL) && ( pc_memberItem->devKey() == devKey() ) && ( pc_memberItem->itemState(IState_c::Local) ) )
    {
      ...
      if ( (pc_memberItem->itemState(IState_c::ClaimedAddress))
        ...
        { // create FilterBox
          getCanInstance4Comm().insertFilter(getProcessInstance4Comm(), 0x7F0, ui16_filter, false, Ident_c::StandardIdent);
          b_configure = true;
        }
