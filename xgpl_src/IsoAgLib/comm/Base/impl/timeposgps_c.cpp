  ...
  */
...
#endif
  {
	  ui16_courseOverGroundRad10Minus4 = 65535;
	  ui16_speedOverGroundCmSec = 65535;
	  ui16_headingRad10Minus4 = 65535;
	  ui16_speedCmSec = 65535;
  }
  ...
  void TimePosGPS_c::checkCreateReceiveFilter( )
  {
    ...
    { // check if needed receive filters for ISO are active
      ...
      c_can.insertFilter(*this, 0x1FFFF00UL,
                    (static_cast<MASK_TYPE>(NMEA_GPS_POSITON_RAPID_UPDATE_PGN) << 8), false, Ident_c::ExtendedIdent);

      // *************************************************************************************************
      // GNSS SOG/COG
      // mask: (0x1FFFF << 8) filter: (NMEA_GPS_COG_SOG_RAPID_UPDATE_PGN << 8)
      c_can.insertFilter(*this, 0x1FFFF00UL,
                    (static_cast<MASK_TYPE>(NMEA_GPS_COG_SOG_RAPID_UPDATE_PGN) << 8), true, Ident_c::ExtendedIdent);
...
void TimePosGPS_c::init(const DevKey_c* rpc_devKey, IsoAgLib::IdentMode_t rt_identMode)
{
  ...
  bool TimePosGPS_c::configGps(const DevKey_c* rpc_devKey, IsoAgLib::IdentMode_t  rt_identModeGps)
  {
    ...
    { // IdentModeImplement
      ...
      getMultiReceiveInstance4Comm().registerClient(NMEA_GPS_POSITON_DATA_PGN,   0xFF, this, true, false, false); // normal MP
      getMultiReceiveInstance4Comm().registerClient(NMEA_GPS_DIRECTION_DATA_PGN, 0xFF, this, true, false, false); // normal MP
      getMultiReceiveInstance4Comm().registerClient(NMEA_GPS_POSITON_DATA_PGN,   0xFF, this, true, false, true);  // NMEA 2000 specific FastPacket
      getMultiReceiveInstance4Comm().registerClient(NMEA_GPS_DIRECTION_DATA_PGN, 0xFF, this, true, false, true);  // NMEA 2000 specific FastPacket
 ...
 int32_t TimePosGPS_c::lastedTimeSinceUpdateGpsCourse() const
 {
	  const int32_t ci32_now = Scheduler_c::getLastTimeEventTrigger();
#ifdef NMEA_2000_FAST_PACKET
	  if ( i32_lastIsoDirectionStream > i32_lastIsoDirectionSimple ) return ( ci32_now - i32_lastIsoDirectionStream);
	  else return ( ci32_now - i32_lastIsoDirectionSimple);
#else
	  return ( ci32_now - i32_lastIsoDirectionSimple);
#endif
  }
  /** Retrieve the time of last update */
  int32_t TimePosGPS_c::lastUpdateTimeGpsCourse() const
  {
#ifdef NMEA_2000_FAST_PACKET
	  if ( i32_lastIsoDirectionStream > i32_lastIsoDirectionSimple ) return i32_lastIsoDirectionStream;
	  else return i32_lastIsoDirectionSimple;
#else
	  return i32_lastIsoDirectionSimple;
#endif
 }
#endif
  ...
  bool TimePosGPS_c::isoProcessMsg()
  {
    ...
    switch (data().isoPgn() & 0x1FFFF)
    {
      ...
      case NMEA_GPS_COG_SOG_RAPID_UPDATE_PGN:
        if ( checkParseReceived( c_tempDevKey ) )
        { // sender is allowed to send
          ui16_courseOverGroundRad10Minus4  = data().getInt16Data( 2 );
          ui16_speedOverGroundCmSec = data().getInt16Data( 4 );
          // set last time
          i32_lastIsoDirectionSimple = ci32_now;
//          c_sendGpsDevKey = c_tempDevKey;
          setUpdateTime( ci32_now );
          // indicate that the message has been processed
          return true;
