...
void CANIO_c::CheckSetCntMsgObj(){
  ...
  { // more objects than allowed are in arrMsgObj, because intervall between
    ...
    { // we have to merge the found min bit difference because:
      ...
      if (pc_minLeft->filter().identType() == Ident_c::StandardIdent)
      {
        pc_useGlobalMask = &c_maskStd;
        c_maskStd.set( c_maskStd.ident() & (~( pc_minLeft->filter().ident() ^ pc_minRight->filter().ident() ) ), Ident_c::StandardIdent );
      }
      else
      {
