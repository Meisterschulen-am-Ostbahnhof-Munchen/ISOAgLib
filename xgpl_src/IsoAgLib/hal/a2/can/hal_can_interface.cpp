...
void updateCanStateTimestamps(uint8_t rui8_busNr)
{
  ...
  getCanBusStatus(rui8_busNr, &t_cinterfCanState); //BW
...
void updateSuccSendTimestamp(uint8_t rui8_busNr)
{
  ...
  for (uint8_t ui8_ind = 0; ui8_ind < 14; ui8_ind++)
  {
    ...
    i16_actCnt = getCanMsgBufCount (rui8_busNr, (ui8_ind+1)); //BW
    if ((i16_actCnt >= 0) &&
...
bool can_stateMsgobjSendproblem(uint8_t rui8_busNr, uint8_t rui8_msgobjNr)
{
  ...
  if (((i32_now - can_stateMsgobjTxok(rui8_busNr, rui8_msgobjNr)) > CONFIG_CAN_MAX_SEND_WAIT_TIME)
    && ( getCanMsgBufCount(rui8_busNr, (rui8_msgobjNr+1)) > 0 ))   //BW
    return true;
  else return false;
}
...
{ // map rui8_msgobjNr 1 to 0
  ...
  // whereas IsoAgLib starts with 0
  //BW
  if (getCanObjStatus(rui8_busNr, (rui8_msgobjNr+1), &t_cinterfMsgobjState) == HAL_NO_ERR)
  {
...
int16_t can_stateMsgobjBuffercnt(uint8_t rui8_busNr, uint8_t rui8_msgobjNr)
{
  // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  // whereas IsoAgLib starts with 0
  return getCanMsgBufCount(rui8_busNr, (rui8_msgobjNr+1));	 //BW
}
...
{ // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  // whereas IsoAgLib starts with 0
  int16_t i16_msgcnt = getCanMsgBufCount(rui8_busNr, (rui8_msgobjNr+1));//BW
...
int16_t can_configGlobalInit(uint8_t rui8_busNr, uint16_t rb_baudrate, uint16_t rb_maskStd, uint32_t rui32_maskExt, uint32_t rui32_maskLastmsg)
{
  ...
  // now config BUS
  //BW
  return initCan(rui8_busNr, rb_maskStd, rui32_maskExt, rui32_maskLastmsg, rb_baudrate);
}
...
int16_t can_configGlobalMask(uint8_t rui8_busNr, uint16_t rb_maskStd, uint32_t rui32_maskExt, uint32_t rui32_maskLastmsg)
{
  ...
  // configured CAN BUS
  //BW
  int16_t i16_retVal = initCan(rui8_busNr, rb_maskStd, rui32_maskExt, rui32_maskLastmsg, t_cinterfCanState.wBitrate);
...
//BW    pt_config->pfIrqFunction = 0;
  ...
  // whereas IsoAgLib starts with 0
  //BW
  return configCanObj(rui8_busNr, (rui8_msgobjNr+1), pt_config);
}
...
{ // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  ...
  b_canBufferLock[rui8_busNr][rui8_msgobjNr] = false;
  //BW
  return chgCanObjId(rui8_busNr, (rui8_msgobjNr+1), rrefc_ident.ident(), rrefc_ident.identType());
}
...
int16_t can_configMsgobjSendpause(uint8_t rui8_busNr, uint8_t rui8_msgobjNr, uint16_t rui16_minSend)
{
  // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  // whereas IsoAgLib starts with 0
  return chgCanObjPause(rui8_busNr, (rui8_msgobjNr+1), rui16_minSend); //BW
}
...
{ // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  ...
  if ( ( rui8_busNr >= cui32_maxCanBusCnt ) || ( rui8_msgobjNr> 14 ) ) return HAL_RANGE_ERR;
  b_canBufferLock[rui8_busNr][rui8_msgobjNr] = false;
  return closeCanObj(rui8_busNr, (rui8_msgobjNr+1));
}
...
{ // check if some msg were sent from buffer
  ...
  #endif
  const uint16_t freeItems = CONFIG_CAN_SEND_BUFFER_SIZE - __HAL::getCanMsgBufCount( rui8_busNr, (rui8_msgobjNr+1) ); //BW
  ...
  // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  // whereas IsoAgLib starts with 0
  return sendCanMsg(rui8_busNr, (rui8_msgobjNr+1), pt_send);
}
...
int32_t can_useMsgobjReceivedIdent(uint8_t rui8_busNr, uint8_t rui8_msgobjNr, int32_t &reflIdent)
{
  ...
  { // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
    // whereas IsoAgLib starts with 0
    i16_retVal = getCanMsg(rui8_busNr, (rui8_msgobjNr+1), pt_receive);
  }
...
int16_t can_useMsgobjGet(uint8_t rui8_busNr, uint8_t rui8_msgobjNr, __IsoAgLib::CANPkg_c* rpc_data)
{
  ...
  { // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
    // whereas IsoAgLib starts with 0
    i16_retVal = getCanMsg(rui8_busNr, (rui8_msgobjNr+1), pt_receive); //BW
  }
...
int16_t can_useMsgobjClear(uint8_t rui8_busNr, uint8_t rui8_msgobjNr)
{
  // add offset 1 to rui8_msgobjNr as ESX BIOS starts counting with 1
  // whereas IsoAgLib starts with 0
  return clearCanObjBuf(rui8_busNr, (rui8_msgobjNr+1));//BW
}
} // end of namespace
