...
{
  bool b_overflow = (rui8_msgobjNr < 15)?false:true;
    updateSuccSendTimestamp(rui8_busNr);
  return b_overflow;
}
...
{
  getCanBusStatus(rui8_busNr, &t_cinterfCanState);
  int16_t i16_retVal = changeGlobalMask(rui8_busNr, rb_maskStd, rui32_maskExt, rui32_maskLastmsg);
...
{ // check if some msg were sent from buffer
  ...
  rpc_data->getData((uint32_t&)pt_send->dwId, pt_send->bXtd, pt_send->bDlc, pt_send->abData);
