 ...
 * In the examples it is demostrated the use of the IsoAgLib by two threads which
 * have a mutual exclusive access to the resource.
 * This example can work on PC linux environment and ESX target environment.
 ...
 *  <li>Mutual exclusive access of the resource IsoAglib by two threads(/ESX task)
 * </ul>
 ...
 *     conf_6_0_VtThreads from the directory where the spec file
 *     and the script are located<br>
 *     <i>#> update_makefile.sh conf_6_0_VtThreads</i>
 * <li><b>Only Linux:</b><ul>
 *     <li><i>#>cd 6_0_VtThreads</i> to go to the subdirectory 6_0_VtThreads
 *     <li><i>#> make</i><br>
 ...
 *		<li> \ref PrjSpec6_0_VtThreads__pc_win32__can_server_sock__simulating
 *		<li> \ref PrjSpec6_0_VtThreads__pc_win32__can_server_sock__sys
 ...
 * - Configuration Setups for Linux on PC:
 *    - \ref FileLists6_0_VtThreads__pc_linux__msq_server_no_card__rte
 *		<li> \ref FileLists6_0_VtThreads__pc_linux__msq_server_rte__rte
 *		<li> \ref FileLists6_0_VtThreads__pc_linux__msq_server_A1__rte
 *		<li> \ref FileLists6_0_VtThreads__pc_linux__msq_server_pcan__rte
 *    - \ref FileLists6_0_VtThreads__pc_linux__msq_server_no_card__simulating
 ...
 * - Configuration Setups for Win32 on PC:
 *    - \ref FileLists6_0_VtThreads__pc_win32__can_server_sock__simulating
 *    <li> \ref FileLists6_0_VtThreads__pc_win32__can_server_sock__sys
 * - Configuration Setupts for some embedded targets:
...
#include <pthread.h>
void* thread1_changeColor(void* ptr);
void* thread2_changeFill(void* ptr);
#else

void thread1_changeColor(void);
void thread2_changeFill(void);

const int ci_taskTimeTask1 = 1000;         // interval task 1 = 1 * 10 ms
const int ci_taskTimeTask2 = 1500;         // interval task 2 = 5 * 10 ms = 50 ms
...
int main()
{
  ...
  */
  ...
  int iret1 = pthread_create(&thread1,NULL,thread1_changeColor,(void*) NULL);
  int iret2 = pthread_create(&thread2,NULL,thread2_changeFill,(void*) NULL);
#else

  HAL::initTaskCall(TASKLEVEL_1A,ci_taskTimeTask1,100, thread1_changeColor);
  HAL::initTaskCall(TASKLEVEL_2A,ci_taskTimeTask2,200, thread2_changeFill);
#endif
  ...
  { // run main loop
	...
	#ifdef SYSTEM_PC
     INTERNAL_DEBUG_DEVICE << "MAIN" << INTERNAL_DEBUG_DEVICE_ENDL;
	#endif

    i32_idleTimeSpread = IsoAgLib::getISchedulerInstance().timeEvent();
    if(i32_idleTimeSpread == iScheduler_c::resourceLocked)
    {
	#ifdef SYSTEM_PC
      INTERNAL_DEBUG_DEVICE << "TimeEvent_LOCKED" << INTERNAL_DEBUG_DEVICE_ENDL;
      i32_idleTimeSpread = IsoAgLib::getISchedulerInstance().timeEventWithWaitMutex();
	#endif
    }

	#ifdef SYSTEM_PC
    INTERNAL_DEBUG_DEVICE << "end MAIN" << INTERNAL_DEBUG_DEVICE_ENDL;
	#endif
...
#endif
{
  ...
  { // run main loop
    ...
    i32_mutexResult = IsoAgLib::getISchedulerInstance().tryAcquireResource();
    #ifdef SYSTEM_PC
    if ( i32_mutexResult != 0 )
	{ // we didn't get the Mutex
	  // wait until we get the mutex -> as soon as this call comes back, we can access IsoAgLib
      i32_mutexResult = IsoAgLib::getISchedulerInstance().waitAcquireResource();
	}
    #endif
    if ( i32_mutexResult == 0 )
    { // we got exclusive access to IsoAgLib Mutex -> now we can safely access any IsoAgLib data structure
	  #ifdef SYSTEM_PC
      INTERNAL_DEBUG_DEVICE << "THREAD_1 modifies the Color" << INTERNAL_DEBUG_DEVICE_ENDL;
	  #endif
      // the following function call triggers some indirect changes and accesses of IsoAgLib
      // content - here: send some command CAN packages to VT
      Tutorial_3_0_Pool_c.eventKeyCode ( KEY_HAS_BEEN_PRESSED, 0, 0, vtKeyCodeKeyChangeCol, 0 );

	  #ifdef SYSTEM_PC
      INTERNAL_DEBUG_DEVICE << "end THREAD_1" << INTERNAL_DEBUG_DEVICE_ENDL;
	  #endif
      // no further access of IsoAgLib data structures needed -> so release the Mutex now
      // so that the main thread can trigger the normal Scheduler_c::timeEvent() based activities again
...
#endif
{
  ...
  { // run main loop
    ...
    { // we got exclusive access to IsoAgLib Mutex -> now we can safely access any IsoAgLib data structure
      ...
      // the following function call triggers some indirect changes and accesses of IsoAgLib
      // content - here: send some command CAN packages to VT
      Tutorial_3_0_Pool_c.eventKeyCode ( KEY_HAS_BEEN_PRESSED, 0, 0, vtKeyCodeKeyChangeFill, 0 );
