            ...
            switch (commandType)
            {
              ...
              case ctChangeStringValue:
                if(child->hasAttributes())
                {
                  ...
                  sprintf(commandMessage, "0xB3, %d, %d, %d, %d%s", MACRO_16bitToLE(idOrName_toi(attrString [attrObjectID], /*macro?*/false)), MACRO_16bitToLE(atoi(attrString [attrBytes_in_string])), tempString2);
