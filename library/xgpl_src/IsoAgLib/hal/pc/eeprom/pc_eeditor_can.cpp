    ...
    while ( fgets( pcZeile, 49, eeditor ) != NULL ) {
      ...
      switch ( pcType ) {
        case 'l':
          ...
          HAL::eepromWrite( wAddress, 4, ( const uint8_t* )&lVal );
          break;
        case 'i':
          sscanf( pcVal, "%hd", &iVal );
          printf( "EEprom Editor Int16 %hd and Addresse %d\n", iVal, wAddress );
          HAL::eepromWrite( wAddress, 2, ( const uint8_t* )&iVal );
          break;
        case 'c':
          sscanf( pcVal, "%hd", &iVal );
          cVal = iVal;
          printf( "EEprom Editor int8_t %c and Addresse %d\n", cVal, wAddress );
          HAL::eepromWrite( wAddress, 1, ( const uint8_t* )&cVal );
          break;
        case 'f':
          sscanf( pcVal, "%f", &fVal );
          printf( "EEprom Editor Float %f and Addresse %d\n", fVal, wAddress );
          HAL::eepromWrite( wAddress, 4, ( const uint8_t* )&fVal );
          break;
        case 's':
          printf( "EEprom Editor Text %s and Addresse %d\n", pcVal, wAddress );
          HAL::eepromWrite( wAddress, wLen, ( const uint8_t* )pcVal );
          break;
      }
    }
    return HAL_NO_ERR;
