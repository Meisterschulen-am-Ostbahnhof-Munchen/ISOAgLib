...
#  endif
  ...
  int16_t openDatFileAndSeek( long al_position ) {
    ...
    if ( NULL == eepromDat ) { // try to create file with creation mode
      ...
      if ( NULL != eepromDat ) { // we have to close the file, as the normal operation works only with a "r+b" opened FILE
    ...
    if ( NULL == eepromDat ) { // have another try with file directly in calling directory
    ...
    if ( NULL != eepromDat ) { // seek on success to the wanted position
