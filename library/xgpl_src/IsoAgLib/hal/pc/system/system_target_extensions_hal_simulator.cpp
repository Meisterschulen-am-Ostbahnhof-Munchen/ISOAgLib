...
#error "LINUX_VERSION_CODE is not defined"
  ...
  clock_t getStartUpTime() {
    ...
    if ( msecPerClock != ( 1000 / sysconf( _SC_CLK_TCK ) ) ) { // BIG PROBLEM -> THE DEFINE DURING COMPILE TIME DOES NOT MATCH THE RUNTIME
  ...
  // msec timer - use that
  int32_t getStartupTime() { // returns time in msec
  ...
  */
  int16_t open_system() { // init system start time
    getTime();
  ...
  */
  int16_t closeSystem( void ) { // remember if are are already talked
  ...
  */
  ...
  // VC++ with native Win32 API provides very accurate
  // msec timer - use that
  int32_t getTime() { // returns time in msec
...
// use gettimeofday for native LINUX system
  int32_t getTime() {
    ...
    if (( ci64_deltaChange >= 1000 ) || ( ci64_deltaChange <= -1000 ) ) { // user changed the system clock inbetween
  ...
  void powerDown( void ) {
    if ( getOn_offSwitch() == 0 ) { // CAN_EN is OFF -> stop now system
      can_stopDriver();
  ...
  int16_t getOn_offSwitch( void ) {
    return HALSimulator_c::GetHALSimulator_c().getOn_offSwitch();
  }
