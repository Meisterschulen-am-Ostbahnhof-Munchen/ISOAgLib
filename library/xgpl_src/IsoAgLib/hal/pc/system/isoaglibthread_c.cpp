...
namespace __IsoAgLib {
  ...
  IsoAgLibThread_c::start( void *key, uint8_t aui8_busNr ) {
    ...
    if ( cb_wasNewlyInserted ) { /// key was not yet registered, but is now
#ifdef DEBUG_THREAD
      std::cout << "IsoAgLibThread_c::start - Newly added key " << key << "." << std::endl;
#endif
      if ( cb_wasRunning ) { /// was already running before insertion
        if ( aui8_busNr == __IsoAgLib::getCanInstance().getBusNumber() ) { // same setup
          return startSuccess;
        }
      ...
      else { /// was not yet running (before insertion)
  ...
  IsoAgLibThread_c::stop( void *key ) {
    ...
    if ( mset_keys.erase( key ) == 0 ) { /// was not started for this key at all
      return stopFailNotStartedForThisKey;
    }
    else { /// was running for this key - key is now already erased
      if ( !isRunning() ) { /// No more keys -> stop thread then of course, too!
#ifdef DEBUG_THREAD
      ...
      else { // still some other key(s) registered, keep thread running
#ifdef DEBUG_THREAD
  ...
  IsoAgLibThread_c::thread_core( void* thread_param ) {
    ...
    while ( !thisThread.mb_requestThreadToStop ) {
      ...
      if ( i32_sleepTime > 0 ) { // We can sleep...
        if ( i32_sleepTime > 3 ) { /// @todo TIMING As long as the GUI thread won't wake up the "waitUntilCanReceiveOrTimeout()"-call below, lets simply limit the sleep time
          i32_sleepTime = 3;
        }
