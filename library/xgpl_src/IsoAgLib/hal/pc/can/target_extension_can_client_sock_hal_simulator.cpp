/*
  target_extension_can_client_sock_hal_simulator.cpp: source for HAL
    simulator for CAN communication This file was based on
  ...
  int32_t send_command( transferBuf_s* p_writeBuf, SOCKET_TYPE ri32_commandSocket ) {
    ...
    if ( p_writeBuf->ui16_command == COMMAND_ACKNOWLEDGE ) {
      if (( p_writeBuf->s_acknowledge.i32_dataContent == ACKNOWLEDGE_DATA_CONTENT_ERROR_VALUE )
          && ( p_writeBuf->s_acknowledge.i32_data != 0 ) ) { // error!
        printf( "NACK received in send_command\n" );
        return p_writeBuf->s_acknowledge.i32_data;
      }
      else { // either ACK with NO error OR ACK with PIPE_ID, SEND_DELAY, etc.etc. which is okay!
        return HAL_NO_ERR;
      }
    }
    else { // wrong answer to a COMMAND. Must be COMMAND_ACKNOWLEDGE!
...
//  changed to only one through this file, and the one defined externally.
  int16_t getCanMsg( uint8_t bBusNumber,uint8_t bMsgObj, tReceive * ptReceive ) {
    ...
    HALSimulator_c::GetHALSimulator_c().ReceiveCanMsg( bBusNumber, ptReceive->tReceiveTime.l1ms, ptReceive->bXtd, ptReceive->dwId, ptReceive->bDlc, ptReceive->abData );
  ...
  int16_t sendCanMsg( uint8_t bBusNumber,uint8_t bMsgObj, tSend* ptSend ) {
    ...
    HALSimulator_c::GetHALSimulator_c().SendCanMsg( bBusNumber, s_transferBuf.s_data.i32_sendTimeStamp, ptSend->bXtd, ptSend->dwId, ptSend->bDlc, ptSend->abData );
