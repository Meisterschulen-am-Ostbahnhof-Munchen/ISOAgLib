...
// would equal DEBUG_PRINT8
  ...
  int32_t getMaxSendDelay( uint8_t /* aui8_busNr */ ) {
    ...
    if ( i16_rc == HAL_NO_ERR ) { // we got an answer - see if it was ACKNOWLEDGE_DATA_CONTENT_SEND_DELAY
      if ( s_transferBuf.s_acknowledge.i32_dataContent == ACKNOWLEDGE_DATA_CONTENT_SEND_DELAY ) { // yihaa, we got what we wanted!
        return s_transferBuf.s_acknowledge.i32_data;
      }
    }
