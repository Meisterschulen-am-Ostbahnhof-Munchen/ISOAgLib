  ...
  */
  ...
  /**
    ...
    @return number of messages which can be placed in buffer or negative error code:
      ...
      HAL_RANGE_ERR == wrong BUS or MsgObj number
  */
  int16_t can_stateMsgobjFreecnt( uint8_t aui8_busNr, uint8_t aui8_msgobjNr ) { // add offset 1 to aui8_msgobjNr as ESX BIOS starts counting with 1
    // whereas IsoAgLib starts with 0
  ...
  */
  ...
  /**
    ...
    @return HAL_NO_ERR == no error;
            ...
            HAL_RANGE_ERR == wrong BUS or MsgObj number
  */
  int16_t can_configMsgobjClose( uint8_t aui8_busNr, uint8_t aui8_msgobjNr ) { // add offset 1 to aui8_msgobjNr as ESX BIOS starts counting with 1
    // whereas IsoAgLib starts with 0
  ...
  */
  int16_t can_useMsgobjSend( uint8_t aui8_busNr, uint8_t aui8_msgobjNr, __IsoAgLib::CanPkg_c* apc_data ) { // check if some msg were sent from buffer
    ...
    if (( lastSendLen[aui8_busNr][aui8_msgobjNr] == pt_send->bDlc )
        ...
        && ( memcmp( lastSendData[aui8_busNr][aui8_msgobjNr], pt_send->abData, pt_send->bDlc ) == 0 ) ) { // gleich
