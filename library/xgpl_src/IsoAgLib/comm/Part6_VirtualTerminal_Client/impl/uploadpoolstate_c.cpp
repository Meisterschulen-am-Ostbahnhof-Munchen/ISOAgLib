...
UploadPoolState_c::initObjectPoolUploadingPhases(
  ...
  const std::vector<IsoAgLib::iVtObject_c*>* rppc_listOfUserPoolUpdateObjects,
  uint16_t aui16_numOfUserPoolUpdateObjects )
{
  ...
  { // Activate User triggered Partial Pool Update
    ...
    {
    	IsoAgLib::iVtObject_c* curObjectO =  mppc_uploadPhaseUserObjects->at(curObject);
    	vtObject_c* curObjectOO = (vtObject_c*)curObjectO;
        ms_uploadPhaseUser.ui32_size += fitTerminalWrapper( curObjectOO );
    }
  }
  else
  { // *CONDITIONALLY* Calculate GENERAL Parts sizes
    ...
    { // start with first phase
      ...
      for (uint32_t curObject=0; curObject < m_pool.getNumObjects(); ++curObject)
        ms_uploadPhasesAutomatic [UploadPhaseIVtObjectsFix].ui32_size += fitTerminalWrapper( (vtObject_c*) m_pool.getIVtObjects(0)->at(curObject)  );
    ...
    if( m_pool.multiLanguage() )
    {
      ...
      for (uint32_t curObject=0; curObject < m_pool.getNumObjectsLang(); ++curObject)
        ms_uploadPhasesAutomatic[ UploadPhaseIVtObjectsLang ].ui32_size
          += fitTerminalWrapper( (vtObject_c*) m_pool.getIVtObjects(realUploadingLanguageAsIndex)->at(curObject) );
