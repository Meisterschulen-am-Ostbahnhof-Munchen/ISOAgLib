...
#define MACRO_vtObjectTypeS iVtObjectAuxiliaryFunction_s
  ...
  if (sourceOffset == 0) { // dump out constant sized stuff
    ...
    destMemory [3] = __IsoAgLib::getVtClientInstance4Comm().getClientByID (s_properties.clientId).getUserConvertedColor (vtObjectAuxiliaryFunction_a->backgroundColour, this, IsoAgLib::BackgroundColour);
...
{
  MACRO_localVars;
  return genericChangeChildPosition (apc_childObject, x, y, b_updateObject, vtObjectAuxiliaryFunction_a->numberOfObjectsToFollow, const_cast<IsoAgLib::repeat_iVtObject_x_y_iVtObjectFontAttributes_row_col_s *> (vtObjectAuxiliaryFunction_a->objectsToFollow), MACRO_getStructOffset(get_vtObjectAuxiliaryFunction_a(), objectsToFollow), sizeof(iVtObjectAuxiliaryFunction_s), b_enableReplaceOfCmd, SoftKeyOffset);
}
