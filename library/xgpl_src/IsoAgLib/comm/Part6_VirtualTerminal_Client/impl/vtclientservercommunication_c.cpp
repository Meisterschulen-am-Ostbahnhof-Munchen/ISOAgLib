...
#define MACRO_setStateDependantOnError(errByte) \
  ...
  switch (arrui8_canData [0])
  {
    ...
    case 0x01: // Command: "Control Element Function", parameter "Button"
      mc_streamer.mrc_pool.eventKeyCode (arrui8_canData [1] /* key activation code (pressed, released, held) */,
    ...
    case 0x02: // Command: "Control Element Function", parameter "Pointing Event"
      mc_streamer.mrc_pool.eventPointingEvent (arrui8_canData [1] | (arrui8_canData [2] << 8) /* X position in pixels */,
                                               arrui8_canData [3] | (arrui8_canData [4] << 8) /* Y position in pixels */);
      break;

    case 0x03: // Command: "VT Select Input Object"
      mc_streamer.mrc_pool.eventVtSelectInputObject(uint16_t(arrui8_canData [1]) | (uint16_t(arrui8_canData [2]) << 8) /* objID */,
                                                    arrui8_canData [3],
                                                    arrui8_canData [4]);
      break;
    ...
    case 0x04: // Command: "Control Element Function", parameter "VT ESC"
        ...
        if (arrui8_canData [3] != 0x0)
          ...
          mc_streamer.mrc_pool.eventVtESC(uint16_t(arrui8_canData [1]) | (uint16_t(arrui8_canData [2]) << 8));

        break;
    case 0x05: // Command: "Control Element Function", parameter "VT Change Numeric Value"
      mc_streamer.mrc_pool.eventNumericValue (uint16_t(arrui8_canData [1]) | (uint16_t(arrui8_canData [2]) << 8) /* objID */,
    ...
    case 0x08:  // Command: "Control Element Function", parameter "VT Input String Value"
      if (arrui8_canData [3] <= 4) //within a 8 byte long cmd can be only a 4 char long string
      {
        ...
        mc_streamer.mrc_pool.eventStringValue (uint16_t(arrui8_canData [1]) | (uint16_t(arrui8_canData [2]) << 8) /* objID */,
    ...
    { /* if Waiting or Timedout (or Failed <shouldn't happen>) */
      ...
      { /// Our command was successfully sent & responded to, so remove it from the queue
        ...
        { /* okay, right response for our current command! */
          ...
          mc_streamer.mrc_pool.eventCommandResponse (ui8_uploadCommandError, &(arrui8_canData[0])); // pass "ui8_uploadCommandError" in case it's only important if it's an error or not. get Cmd and all databytes from "mc_data.name()"
#ifdef DEBUG
