...
MultiReceiveClientWrapper_s::start (CanCustomer_c& apc_fpCustomer)
{
  ...
  { /// Fast-Packet additions
    if (!getCanInstance4Comm().existFilter (apc_fpCustomer, (mui32_pgnMask << 8), (mui32_pgn << 8), __IsoAgLib::Ident_c::ExtendedIdent))
      getCanInstance4Comm().insertFilter (apc_fpCustomer, (mui32_pgnMask << 8), (mui32_pgn << 8), true, Ident_c::ExtendedIdent, 8);
  }
  #endif
}
...
MultiReceiveClientWrapper_s::stop (CanCustomer_c& apc_fpCustomer)
{
  ...
  if (mb_isFastPacket)
  { /// Fast-Packet additions
    __IsoAgLib::getCanInstance4Comm().deleteFilter (apc_fpCustomer, (mui32_pgnMask << 8), (mui32_pgn << 8), __IsoAgLib::Ident_c::ExtendedIdent);
  }
  #endif
}
