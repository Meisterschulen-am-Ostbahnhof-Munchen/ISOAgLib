...
bool MyInternalCanHandler_c::processMsg()
{
	...
	switch ( c_myData.ident() )
	{
		case InternalSpeed:
			...
			// signed integer
			c_base.setSpeedReal(  c_myData.getInt16Data( 0 ) );
			c_base.setSpeedTheor( c_myData.getInt16Data( 2 ) );
			return true;
		case InternalDistance:
			// await real and theor distance in one message with each four byte
			// signed integer
			c_base.setDistReal(  c_myData.getInt32Data( 0 ) );
			c_base.setDistTheor( c_myData.getInt32Data( 4 ) );
			return true;
		case InternalPtoFront:
			...
			// signed integer
			c_base.setPtoFront(                        c_myData.getInt16Data( 0 ) );
			c_base.setPtoFrontEngaged( IsoActiveFlag_t(c_myData.getUint8Data( 2 ) ) );
			c_base.setPtoFront1000(    IsoActiveFlag_t(c_myData.getUint8Data( 3 ) ) );
			c_base.setPtoFrontEconomy( IsoActiveFlag_t(c_myData.getUint8Data( 4 ) ) );
			return true;
		case InternalPtoRear:
			...
			// fivth byte as IsoActiveFlag_t the economy mode
			// signed integer
			c_base.setPtoRear(                        c_myData.getInt16Data( 0 ) );
			c_base.setPtoRearEngaged( IsoActiveFlag_t(c_myData.getUint8Data( 2 ) ) );
			c_base.setPtoRear1000(    IsoActiveFlag_t(c_myData.getUint8Data( 3 ) ) );
			c_base.setPtoRearEconomy( IsoActiveFlag_t(c_myData.getUint8Data( 4 ) ) );
			return true;
		case InternalRpm:
			// await engine RPM as 2 byte signed integer
			c_base.setEngine( c_myData.getInt16Data( 0 ) );
			return true;
		case InternalPower:
			// await key switch state as one byte IsoActiveFlag_t
			// await max possible power time as one byte unsigned integer [minute]
			c_base.setKeySwitch( IsoActiveFlag_t( c_myData.getUint8Data( 0 ) ) );
			c_base.setMaxPowerTime(               c_myData.getUint16Data( 1 ) );
			return true;
