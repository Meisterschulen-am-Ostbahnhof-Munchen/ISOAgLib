...
using namespace __IsoAgLib::HwRS232;

/** set trigger len to flushCmd a cmd puffer */
#define CMD_PUFFER_FLUSH_SIZE 80
#define STATIC_GLOBAL_SEND_PUFF_SIZE 150
#define STATIC_GLOBAL_TEMP_STRING_SIZE 1000
...
static far byte pb_globalHddSendPuff[STATIC_GLOBAL_SEND_PUFF_SIZE];
/** temporary string for interrupted (sync_err) write access */
static far byte pb_tempString[STATIC_GLOBAL_TEMP_STRING_SIZE];
  ...
  { // success -> set state to inactive (ready for activation)
    ...
    switch (pb_sendPuff[3])
    {
      ...
      case 23: // read file
        ...
        for (byte b_ind = 4; b_ind <   c_puffResp.size(); b_ind++)
        { // copy text to pc_readFilePuffer
          pc_readFilePuffer[b_readFilePufferPos] = c_puffResp[b_ind];
          b_readFilePufferPos++;
        }
        // check if EOF reached
        if ((getRespLen() - 4) < (pb_sendPuff[6] + (pb_sendPuff[7] << 8)))
...
{ // check actual state of RS232_IO -> allowed for state openableState and closingState
  ...
  {
    while ( (en_hddState == open_state) && (wTempLen > 0) && (en_hddErr == noErr) )
    {
      write(NULL, 0);
      timeEventInternControl(true);
    }
    // repeat to be shure that no text has to be sent
    write(NULL, 0);
    timeEventInternControl(true);
  }
...
{ // first check if write has something to do
  ...
  word wFreeCntInCmd =
    (CMD_PUFFER_FLUSH_SIZE > c_puffCmd.size())?(CMD_PUFFER_FLUSH_SIZE - c_puffCmd.size()):0;
  if (wTempLen > 0)
  {
    word wInd = 0,
      wLimitLoop = wTempLen;
...
/**
  ...
  @param rwLen length of data string
*/
void RS232_IO::send(far byte* rpb_data, word rwLen)
{
...
{ // check if file is opened in read mode
  ...
  { // eat till
    while ((isspace(pc_linePointer[b_ind])) && (b_ind < b_readFilePufferPos))
    {
  ...
  {
    if (
        (pc_linePointer[b_ind] == rc_delim)
     || ((rc_delim == ISSPACE_DELIM) && (isspace(pc_linePointer[b_ind])))
       )
    { // delim found
  ...
  if (b_ind < rb_maxLen) pc_linePointer[b_ind+1] = '\0';

  // now move rest of read puffer to begin of puffer
  char far *from = pc_readFilePuffer + b_ind,
       far *to = pc_readFilePuffer,
       far *end = (pc_readFilePuffer + b_readFilePufferPos);
