...
# until the setpoint classes for PTO and Move are fully implemented, the setpoint classes are NOT integrated into project files
  ...
  case _"$USE_DIGITAL_INOUT" in
      _rte)
          echo 'digital input/output driver: rte'
          DIGINOUTRTE_NEG=''
          ;;
      _*)
          echo 'digital input/output driver: file'
          DIGINOUTRTE_NEG='!'
          ;;
  esac
  ...
  if [ $PRJ_ACTOR -gt 0 ] ; then
      DRIVER_FEATURES="$DRIVER_FEATURES -o -path '*/driver/actor*' -o \\( -path '*/hal/"$HAL_PATH"/actor/*' $DIGINOUTRTE_NEG -name '*_rte.*' \\) -o -path '*/hal/actor.h'"
  fi
  ...
  if [ $PRJ_SENSOR_COUNTER -gt 0 ] ; then
    ...
    DRIVER_FEATURES="$DRIVER_FEATURES -o \\( -path '*/hal/"$HAL_PATH"/sensor/*' $DIGINOUTRTE_NEG -name '*_rte.*' \\) -o -name '*sensorbase_c.*' -o -name '*sensor_c.*' -o -name '*sensori_c.*' -o -path '*/hal/sensor.h'"
	fi
...
#  if [ -n "$APP_NAME" ] ; then
#  	if [ $USE_TARGET_SYSTEM == "pc_linux" ] ; then
#			echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ -follow -name "$APP_NAME" -printf 'SOURCES += %h/%f\n' >> $FILELIST_QMAKE" >> .exec.tmp
#  	fi
# 		echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ -follow -name "$APP_NAME" -printf '%h/%f\n' >> $FILELIST_PURE" >> .exec.tmp
#	else
#  	if [ $USE_TARGET_SYSTEM == "pc_linux" ] ; then
#			echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ $APP_SEARCH_SRC_CONDITION -follow -printf 'SOURCES += %h/%f\n' >> $FILELIST_QMAKE" >> .exec.tmp
#			echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ $APP_SEARCH_HDR_CONDITION -follow -printf 'HEADERS += %h/%f\n' >> $FILELIST_QMAKE" >> .exec.tmp
#		fi

# 		echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ $APP_SEARCH_SRC_CONDITION -follow -printf '%h/%f\n' >> $FILELIST_PURE" >> .exec.tmp
#  	echo "find ../$ISO_AG_LIB_PATH/$REL_APP_PATH/ $APP_SEARCH_HDR_CONDITION -follow -printf '%h/%f\n' >> $FILELIST_HDR" >> .exec.tmp
#  fi
...
function create_makefile()
{
  ...
  for EACH_REL_APP_PATH in $REL_APP_PATH ; do
	...
	echo "" >> $MakefileNameLong
	if [ _"$USE_CAN_DRIVER" = _rte -o _"$USE_RS232_DRIVER" = _rte -o _"$USE_DIGITAL_INOUT" = _rte ] ; then
